...
export default function backupData(app, req) {
    ...
    (async () => {
      try {
        ...
        if (_shouldBackup) {
          ...
          exec(
            `cd ${process.env.BACKUP_DIR_PATH} && git add . && git commit -m "v${appInfo.version}${userId ? ` - Published by User ID: ${userId}"` : ''} && git push origin master`,
            (error, stdout, stderr) => {
            if (error) {
