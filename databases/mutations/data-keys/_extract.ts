...
export function scrapDataKeys({
    ...
    screens: typeof schema.screens.$inferSelect[];
    diagnoses: typeof schema.diagnoses.$inferSelect[];
    drugsLibrary: typeof schema.drugsLibrary.$inferSelect[];
}) {
    let extractedKeys: DataKey[] = [];
    
    console.log('Scrapping screens keys...');
    screens.forEach(s => {
        if (s.key) {
    ...
    console.log('Scrapping diagnoses keys...');
    diagnoses.forEach(d => {
        if (d.key) {
            extractedKeys.push({ 
                id: undefined,
    ...
    console.log('Scrapping drugsLibrary keys...');
