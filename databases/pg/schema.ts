...
export const users = pgTable(
  "nt_users",
  {
    ...
    role: roleNameEnum("role")
      .references(() => userRoles.name, { onDelete: "cascade" })
      .default("user")
      .notNull(),
  ...
  (table) => ({
    searchIndex: index("users_search_index").using(
      "gin",
      sql`(
...
export const hospitals = pgTable(
  ...
  (table) => ({
    searchIndex: index("hospitals_search_index").using(
      "gin",
      sql`(
...
// DEVICES
export const devices = pgTable("nt_devices", {
  id: serial("id").primaryKey(),
...
export const filesChunks = pgTable("nt_files_chunks", {
  id: serial("id").primaryKey(),
...
export const mdmProviderProfiles = pgTable("nt_mdm_provider_profiles", {
  ...
  providerIndex: index("mdm_provider_profiles_provider_idx").on(table.provider),
  countryIndex: index("mdm_provider_profiles_country_idx").on(table.countryISO),
}))

// DEVICE MDM LINKS
export const deviceMdmLinks = pgTable("nt_device_mdm_links", {
  id: serial("id").primaryKey(),
  linkId: uuid("link_id").notNull().unique().defaultRandom(),

  deviceId: text("device_id").notNull().references(() => devices.deviceId, { onDelete: "cascade" }),
  provider: mdmProviderEnum("provider").default("headwind").notNull(),
  profileId: uuid("profile_id").references(() => mdmProviderProfiles.profileId, { onDelete: "set null" }),
  ...
  deviceProviderUnique: uniqueIndex("device_mdm_link_device_provider_unique").on(table.deviceId, table.provider),
...
export const mdmProviderProfilesRelations = relations(mdmProviderProfiles, ({ one, many }) => ({
  hospital: one(hospitals, {
    fields: [mdmProviderProfiles.hospitalId],
    references: [hospitals.hospitalId],
  }),
...
export const deviceMdmLinksRelations = relations(deviceMdmLinks, ({ one }) => ({
  device: one(devices, {
    fields: [deviceMdmLinks.deviceId],
...
export const configKeys = pgTable(
  ...
  (table) => ({
    searchIndex: index("config_keys_search_index").using(
      "gin",
      sql`(
...
// CONFIG KEYS DRAFTS
export const configKeysDrafts = pgTable("nt_config_keys_drafts", {
  id: serial("id").primaryKey(),
...
// CONFIG KEYS HISTORY
export const configKeysHistory = pgTable("nt_config_keys_history", {
  id: serial("id").primaryKey(),
  version: integer("version").notNull(),
  configKeyId: uuid("config_key_id")
    .references(() => configKeys.configKeyId, { onDelete: "cascade" })
    .notNull(),
  restoreKey: uuid("restore_key"),
  changes: jsonb("data").default([]),
...
export const scripts = pgTable(
  ...
  (table) => ({
    searchIndex: index("scripts_search_index").using(
      "gin",
      sql`(
...
// SCRIPTS DRAFTS
export const scriptsDrafts = pgTable("nt_scripts_drafts", {
  id: serial("id").primaryKey(),
...
// SCRIPTS HISTORY
export const scriptsHistory = pgTable("nt_scripts_history", {
  id: serial("id").primaryKey(),
  version: integer("version").notNull(),
...
export const screens = pgTable(
  ...
  (table) => ({
    searchIndex: index("screens_search_index").using(
      "gin",
      sql`(
                    to_tsvector('english', ${table.title})
                )`,
...
// SCREENS DRAFTS
export const screensDrafts = pgTable("nt_screens_drafts", {
  id: serial("id").primaryKey(),
...
// SCREENS HISTORY
export const screensHistory = pgTable("nt_screens_history", {
  id: serial("id").primaryKey(),
  version: integer("version").notNull(),
  screenId: uuid("screen_id")
    .references(() => screens.screenId, { onDelete: "cascade" })
    .notNull(),
  scriptId: uuid("script_id")
    .references(() => scripts.scriptId, { onDelete: "cascade" })
    .notNull(),
...
export const diagnoses = pgTable(
  ...
  (table) => ({
    searchIndex: index("diagnoses_search_index").using(
      "gin",
      sql`(
                    to_tsvector('english', ${table.name})
                )`,
...
// DIAGNOSES DRAFTS
export const diagnosesDrafts = pgTable("nt_diagnoses_drafts", {
  id: serial("id").primaryKey(),
...
// DIAGNOSES HISTORY
export const diagnosesHistory = pgTable("nt_diagnoses_history", {
  id: serial("id").primaryKey(),
  version: integer("version").notNull(),
  diagnosisId: uuid("diagnosis_id")
    .references(() => diagnoses.diagnosisId, { onDelete: "cascade" })
    .notNull(),
  scriptId: uuid("script_id")
    .references(() => scripts.scriptId, { onDelete: "cascade" })
    .notNull(),
...
export const drugsLibrary = pgTable("nt_drugs_library", {
  ...
  ageKeyId: text("age_key_id").notNull().default(""),
  gestationKeyId: text("gestation_key_id").notNull().default(""),
  weightKeyId: text("weight_key_id").notNull().default(""),
  diagnosisKeyId: text("diagnosis_key_id").notNull().default(""),
...
export const drugsLibraryDrafts = pgTable("nt_drugs_library_drafts", {
  id: serial("id").primaryKey(),
...
export const drugsLibraryHistory = pgTable("nt_drugs_library_history", {
  id: serial("id").primaryKey(),
  version: integer("version").notNull(),
  itemId: uuid("item_id")
    .references(() => drugsLibrary.itemId, { onDelete: "cascade" })
    .notNull(),
  restoreKey: uuid("restore_key"),
  changes: jsonb("data").default([]),
