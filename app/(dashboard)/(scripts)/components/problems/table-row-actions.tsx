...
type Props = {
    disabled: boolean;
    problem: Awaited<ReturnType<IScriptsContext['getProblems']>>['data'][0];
...
export function ProblemsTableRowActions({ 
    disabled,
    isScriptLocked,
    scriptLockedByUserId,
    problem: { problemId, isDraft, draftCreatedByUserId, },
    onDelete, 
    onCopy,
}: Props) {
    const { scriptId, } = useParams();

    const editHref = `/script/${scriptId}/problem/${problemId}`;
