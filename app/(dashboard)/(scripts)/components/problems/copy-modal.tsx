...
export function CopyProblemsModal({ 
    open, 
    problemsIds,
    onOpenChange, 
}: {
    ...
    const onCopyProblems = handleSubmit(async (data) => {
        try {
            if (!data.scriptId) throw new Error('Please select a script!');

            setLoading(true);

            // const res = await copyProblems({ 
            //     problemsIds,
            ...
            // TODO: Replace this with server action
            const response = await axios.post('/api/problems/copy', { 
                problemsIds,
                toScriptsIds: [data.scriptId], 
                broadcastAction: true,
            });
            ...
            alert({
                variant: 'success',
                title: 'Success',
                message: 'Problem copied successfully!',
                onClose: () => {
                    resetForm({ scriptId: '', });
                    onOpenChange(false);
                },
            ...
            alert({
                variant: 'error',
                title: 'Error',
                message: 'Failed to copy problem: ' + e.message,
            });
        } finally {
            setLoading(false);
        }
