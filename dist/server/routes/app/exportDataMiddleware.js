...
module.exports = function () {
  return function (req, res) {
    ...
    if (script || configKey) {
      return Promise.all([configKey ? _models.ConfigKey.findOne({
        where: {
          id: configKey
        }
      }) : null, script ? _models.Script.findOne({
        where: {
          id: script
        }
      }) : null, script ? _models.Screen.findAll({
        where: {
          script_id: script
        }
      }) : null, script ? _models.Diagnosis.findAll({
        where: {
          script_id: script
        }
    ...
    if (allScripts || allConfigKeys) {
      Promise.all([allConfigKeys ? _models.ConfigKey.findAll({}) : null, allScripts ? _models.Script.findAll({}) : null, allScripts ? _models.Screen.findAll({}) : null, allScripts ? _models.Diagnosis.findAll({}) : null]).then(function (rslts) {
